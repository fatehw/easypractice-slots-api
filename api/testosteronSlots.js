...
export default async function handler(req, res) {
  ...
  try {
    // Testosteron calendar (643030) — Jakob Bentin
    const calendarId = process.env.EASYPRACTICE_CALENDAR_ID_TESTOSTERON;
    const token = process.env.EASYPRACTICE_TOKEN;

    // Potensmedicin calendar (639747) — same doctor (Jakob Bentin).
    // EasyPractice blocking rules mean bookings in either calendar block the other.
    // We fetch bookings from BOTH so we never offer already-blocked slots.
    // Set EASYPRACTICE_CALENDAR_ID_POTENSMEDICIN=639747 in your Vercel env.
    const linkedCalendarId =
      process.env.EASYPRACTICE_CALENDAR_ID_POTENSMEDICIN || null;
    ...
    // ── Fetch bookings from BOTH calendars in parallel ──
    ...
    // Merge bookings from both calendars
    const bookings = [
    ...
    while (current <= last) {
      ...
      // EasyPractice bug workaround: some days return closed=false with leftover
      // times (e.g. 18:00-23:55) even when disabled in the UI.
      // Treat any day starting at 17:00 or later as closed.
      const openMin = dayStart ? toMinutes(dayStart) : 0;
      const closeMin = dayEnd ? toMinutes(dayEnd) : 0;
      const hasRealisticHours =
        openMin < toMinutes('17:00') && openMin < closeMin;

      if (!isClosed && dayStart && dayEnd && hasRealisticHours) {
